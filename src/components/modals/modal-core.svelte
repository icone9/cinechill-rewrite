...
${
	$modalPosterStore.isModalOpen
		? 'opacity-100 h-screen z-30'
		: 'opacity-0 pointer-events-none z-0 h-0'
}`}
>
	<div
	...
	>
		...
		<div style="padding: 1em 1.5em;" class="Modal__info--wrp">
			...
			<hr
				style=" border-color: rgba(121,121,121,0.7);
      margin: 1em 0;"
				class="Modal__info--line"
			/>
			...
			<div class="text-sm ml-0 m-2 break-all leading-normal font-medium">
				...
				<span class="text-gray-200">{$modalPosterStore.details?.original_language}<span /></span>
			</div>
		</div>
	</div>
</div>
