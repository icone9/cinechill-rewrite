...
<nav class={`fixed w-full navigation z-30 ${y > 100 ? 'bg-[#232426] shadow-md' : ''} ${open ? 'bg-[#232426]' : ''}
`}>
      ...
      <div class="relative flex items-center justify-between py-4">
        ...
        <div class="flex items-center ml-2 md:ml-4">
           ...
           {#if user}
                ...
                class={`z-10 divide-y rounded-lg shadow w-44 bg-gray-700 divide-gray-600 absolute top-16 right-0 ${userDropdownOpen ? '' : 'hidden'}`}>
                    ...
                    <div class="py-2">
                        <form method="POST" action="/login?/logout" use:enhance>
                            <button type="submit" class="block px-4 py-2 text-sm hover:bg-gray-600 text-gray-200 hover:text-white w-full text-start">
                                Sign out
                            </button>
                        </form>
                    </div>
  ...
  <div class={`h-screen mb-4 mt-10 ${open ? '' : 'hidden'}`}
  style="font-family: 'antique-olive,-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica Neue,Arial,Noto Sans,sans-serif,Apple Color Emoji,Segoe UI Emoji,Segoe UI Symbol,Noto Color Emoji';">
      <div class="px-2 pt-2 pb-3 space-y-1 text-5xl tracking-wider font-bold ml-2">
        <a href="/movies"
        class="hover:text-white rounded-md flex items-center px-2 py-2 ">
