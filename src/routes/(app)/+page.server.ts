...
export const load = (async () => {
	const getBanner = async () => {
		const banner = (await discoverWithNetwork('movie', '213')).results[
			randomIntFromInterval(1, 19)
		];
		return {
			title: banner.title,
			overview: banner.overview,
			backdrop_path: `${TMDB_IMAGE_URL}/original${banner.backdrop_path}`
		};
	};

	const getTrendingMedia = async () => {
		const trendingMovies = (await fetchTMDB<{ page: number; results: Movie[] }>('/movie/popular'))
			.results;
		const trendingTvShow = (await fetchTMDB<{ page: number; results: TvShow[] }>('/tv/popular'))
			.results;
	...
	const getTopRatedMedia = async () => {
		const topRatedMovies = (await fetchTMDB<{ page: number; results: Movie[] }>('/movie/top_rated'))
			.results;
		const topRatedTvShow = (await fetchTMDB<{ page: number; results: TvShow[] }>('/tv/top_rated'))
			.results;
